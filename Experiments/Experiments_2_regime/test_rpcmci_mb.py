...
from Method.rpcmci_mb import RPCMCI
                ...
                for var in dataset_size:
                    for switch_thres in list_switch_thres:
                        ...
                        for i in tqdm(range(50)):
                            ...
                            rpcmci = RPCMCI(dataframe=data_frame,  cond_ind_test=ParCorr(), prediction_model=LinearRegression(), seed=2233, verbosity=1)
                            results = rpcmci.run_rpcmci(num_regimes=regime_num, max_transitions=max_transitions, switch_thres=switch_thres, num_iterations=num_iterations, max_anneal=max_anneal,
                            ...
                            if not os.path.exists(os.path.join('../../results', '2regimes', 'results_rpcmci'+suffixe, str(var))):
                                os.makedirs(os.path.join('../../results', '2regimes', 'results_rpcmci'+suffixe, str(var)))


                            open_file = open(os.path.join('../../results', '2regimes', 'results_rpcmci'+suffixe, str(var), 'switch_thres_' + str(switch_thres) + '_alpha_' + str(pc_alpha)
