...
from Method.rcbnb_pa import RCBNBW_noCindex

# path_data = '../Dataset_0.3_lag_1_sta'  # '../Dataset_0.3_lag_1_sta'
dataset_size = [600, 1200] # [500, 600, 800, 1200] # [320, 400, 480, 560]
regime_num = 2
marker = '_regimes'
experiment = 'n'
max_transitions =  3 # 3
list_switch_thres = [0.05] #[0.5]  # [0.1, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8]
            ...
            for pc_alpha in list_pc_alpha:
                ...
                for var in dataset_size:
                    for switch_thres in list_switch_thres:
                        list_err = []
                        list_results = []
                        for i in tqdm(range(50)): # we only take 50 datasets
                            ...
                            rcbnbw = RCBNBW_noCindex(dataframe=data_frame,  cond_ind_test=RobustParCorr(), prediction_model=LinearRegression(), verbosity=1)
                            results = rcbnbw.run_rcbnbw(num_regimes=regime_num, max_transitions=max_transitions, switch_thres=switch_thres, num_iterations=num_iterations, max_anneal=max_anneal,
                            ...
                            # For 2 regimes
                            if not os.path.exists(os.path.join('../../results', '2regimes', 'results_rcbnb'+suffixe, str(var))):
                                os.makedirs(os.path.join('../../results', '2regimes', 'results_rcbnb' + suffixe, str(var)))


                            # For 2 regimes
                            open_file = open(
                                os.path.join('../../results', '2regimes', 'results_rcbnb' + suffixe, str(var),
                                             'switch_thres_' + str(switch_thres) + '_alpha_' + str(pc_alpha)
                                             + '_ann_' + str(max_anneal) + '_niter_' + str(num_iterations) + '_res_pa.pkl'),
                                "wb")
