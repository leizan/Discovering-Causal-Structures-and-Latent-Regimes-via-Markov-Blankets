...
class RPCMCIPLUS(PCMCI):
    ...
    def run_rpcmciplus(self,
            ...
            for lag in range(lag_max):
                if lag == 0:
                    ...
                    for i in range(num_node):
                        for j in range(num_node):
                            if link_temp[i][j][lag] == '-->':  # Check for lag 1 connections
                                dbn.add_edge((i, lag_max - 1 - lag),
                                             (j, lag_max - 1))  # Add edge between nodes at different time steps
        ...
        def _pcmci(tau_min, tau_max, pc_alpha, alpha_level):
            ...
            results = self.run_pcmciplus(tau_min=tau_min, tau_max=tau_max, pc_alpha=pc_alpha)
            graph = results['graph'] 
        ...
        def one_annealing_step(a):
            ...
            for q in range(num_iterations):
                ...
                for k in range(num_regimes):
                    ...
                    for i in range(link_temp.shape[0]):
                        for j in range(link_temp.shape[0]):
                            if link_temp[i,j,0] == 'o-o':
                                if not (j,0) in parents_temp[i]:
                                    parents_temp[i].append((j,0))
