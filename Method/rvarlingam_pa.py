...
class Regimevarlingam(PCMCI):
    ...
    def run_rvarlingam(self,
        ...
        def _varlingam(org_data, selected_samples_k, tau_min, tau_max, pc_alpha, alpha_level):
            """Wrapper around running JPCMCIplus."""
            reg_data = org_data[selected_samples_k,:]
            adjacent_threshold = 0.4
            ...
            model.fit(reg_data)
            res_matrix = model.adjacency_matrices_

            # contruct adjacency matrix
            for lag in range(tau_max+1):
                for i in range(num_nodes):
                    for m in range(num_nodes):
                        if lag == 0 and np.abs(res_matrix[lag,i,m]) >= adjacent_threshold:
                            adj_matrix[i, m, lag] = '-->'
                            adj_matrix[m, i, lag] = '<--'
                        elif np.abs(res_matrix[lag,i,m]) >= adjacent_threshold:
                            adj_matrix[i, m, lag] = '-->'

            # construct parents dictionary
            parents_set = {}
            for index_node in range(num_nodes):
            ...
            for q in range(num_iterations):
                ...
                for k in range(num_regimes):
                    ...
                    try:
                        ...
                        #                                                 tau_min=tau_min,)
                        results_temp, link_temp, parents_temp = _varlingam(org_data=data, selected_samples_k=selected_samples_k,
                                                                        tau_max=int(tau_max),
                                                                        pc_alpha=pc_alpha,
                                                                        alpha_level=alpha_level,
                                                                        tau_min=tau_min,)
