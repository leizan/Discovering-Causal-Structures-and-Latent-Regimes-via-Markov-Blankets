...
class Regimenotears(PCMCI):
    ...
    def __init__(self, dataframe, cond_ind_test=None,
        ...
        if self.prediction_model is None:
            self.prediction_model = sklearn.linear_model.LinearRegression()  # sklearn.svm.SVR()
    ...
    def run_rdynotears(self,
                            ...
                            if link_temp[i][j][lag] == '-->':  # Check for lag 1 connections
                                for m in range(lag_max):
                                    try:
                                        ...
                                        continue
                                        # print('Establish Markov blanket meeting cycle.')
                else:
                    for i in range(num_node):
                        for j in range(num_node):
                            if link_temp[i][j][lag] == '-->':  # Check for lag 1 connections
        ...
        def _dynotears(org_data, selected_samples_k, tau_min, tau_max, pc_alpha, alpha_level):
            ...
            num_nodes = reg_data.shape[1]
            adj_matrix = np.zeros(shape=(num_nodes, num_nodes, tau_max+1), dtype='<U3')
            reg_data = pd.DataFrame(reg_data)
            sm = from_pandas_dynamic(time_series=reg_data, p=tau_max)
            # reconstruct the graph
            for (ele1, ele2) in sm.edges:
                node1, lag1 = ele1.split('_lag')
                node2, lag2 = ele2.split('_lag')
                node1 = int(node1)
                lag1 = int(lag1)
                node2 = int(node2)
                lag2 = int(lag2)
                if lag1+lag2==0:
                    adj_matrix[node1, node2, lag1] = '-->'
                    adj_matrix[node2, node1, lag1] = '<--'
                else:
                    adj_matrix[node1, node2, lag1] = '-->'
            # reconstruct parents_set
            parents_set = {}

            for index_node in range(num_nodes):
                parents_set[index_node] = []
                for lag in range(tau_max+1):
                    for i in np.where(adj_matrix[:,index_node,lag]== '-->')[0]:
                        parents_set[index_node].append((i,-lag))

            return None, adj_matrix, parents_set
        ...
        def one_annealing_step(a):
            ...
            for q in range(num_iterations):
                ...
                for k in range(num_regimes):
                    ...
                    try:
                        ...
                        #                                                 tau_min=tau_min,)
                        results_temp, link_temp, parents_temp = _dynotears(org_data=data, selected_samples_k=selected_samples_k,
                                                                        tau_max=int(tau_max),
                                                                        pc_alpha=pc_alpha,
                                                                        alpha_level=alpha_level,
                                                                        tau_min=tau_min,)
                        # print('Get the causal graph! ')
                    except Exception:
                        traceback.print_exc()
                        error_flag = True
                    ...
                    # print('parents_temp')
                    # print(parents_temp)
                    # if not _is_acyclic(link_temp=link_temp):
                    #     print('Causal graph is cyclic !')
            ...
            if error_flag:
                ...
                return a, objective_opt, None, None, None, gamma_opt, diff_g
